...
const universities = graduates.map (att => {
  return att.university ;
});
...
const contactInfo = graduates.map (att => {
  return `${att.first_name} ${att.email}`
})
...
const lowerCase = zooAnimals.map(zooAnimals => {
  return (zooAnimals.animal_name.toUpperCase())
})
...
*/
const populationTotal = zooAnimals.reduce((sum, att) => {
  return att.population + sum
}, 0);
